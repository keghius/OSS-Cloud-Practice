...
# 3. 특정 서비스의 컨테이너 개수 조정
docker compose up -d --scale web=3

# 4. 실행 중인 컨테이너 상태 확인
docker compose ps

# 5. 서비스 로그 확인 (-f: 실시간 스트리밍)
docker compose logs -f

# 6. 컨테이너 중지 및 관련 리소스(네트워크 등) 삭제
docker compose down
...
### 2. Dockerfile 작성 (`web/Dockerfile`)
```dockerfile
# web/Dockerfile
...
### 1. 스웜 배포용 Docker Compose 파일 작성 (`docker-compose-swarm.yml`)
